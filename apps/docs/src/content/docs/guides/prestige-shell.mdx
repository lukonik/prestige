...
Put serializable shell options such as `github`, `algolia`, and `license` into `prestige.config.ts`. Keep render props like `customHeaderTitle` and `copyright` on `PrestigeShell`.
...
Pass only runtime render functions through the `options` prop:
