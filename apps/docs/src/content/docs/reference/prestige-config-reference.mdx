---
title: Prestige Config Reference
description: Reference for the `prestige.config.ts` file shape.
---

# Prestige Config Reference

`prestige.config.ts` defines the content-facing configuration for a Prestige site. The file is loaded from the app root and should export `defineConfig(...)`.
...
Use `vite.config.ts` to register the plugin itself. For that surface, see [Vite Plugin Reference](/docs/reference/plugin-reference).
...
- Prestige reads this file from `<root>/prestige.config.ts`.
